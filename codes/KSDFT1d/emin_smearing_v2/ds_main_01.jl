...
# ============================================================================
# 1D grid and helper functions (free boundaries, endpoints included)
# ============================================================================

struct Grid1D
    x::Vector{Float64}       # coordinates, from -L/2 to L/2 inclusive
    dx::Float64               # grid spacing = L / (N-1)
    L::Float64                 # total length (from -L/2 to L/2)
...
# ============================================================================
# Hamiltonian operations (finite‑difference, FREE boundaries)
# ============================================================================
...
function find_mu(ε::Vector{Float64}, N_electrons, kT::Float64, g; tol=1e-12)
...
function compute_density(psi::Matrix{Float64}, f::Vector{Float64})
    N, Nstates = size(psi)
    ρ = zeros(N)
    for n in 1:Nstates
        ρ .+= f[n] * psi[:,n].^2
...
function free_energy(psi, f, Tmat, density, V_H, grid, V_ext, kT, g)
    dx = grid.dx
    E_kin = sum(f[n] * Tmat[n,n] for n in 1:length(f))
    E_ext = sum(density .* V_ext) * dx
    E_H = 0.5 * sum(density .* V_H) * dx
    # Exchange‑correlation energy is omitted here (set to zero)
    S = entropy(f, g)
    #println("E_kin  = $(E_kin)")
    #println("E_ext  = $(E_ext)")
...
function preconditioner_psi(grid::Grid1D; ε_cut=0.5)
    N = grid.N
    L = grid.L
    freqs = fftfreq(N, 2π/L)   # approximate (periodic) frequencies
    Tk = 0.5 * freqs.^2
...
# ============================================================================
# Main conjugate‑gradient minimization (with degeneracy and free boundaries)
# ============================================================================
...
function minimize_dft(;
    ...
    for iter in 1:maxiter
        ...
        ρ_trial = compute_density(psi_rot, f_trial)
        V_H_trial = hartree_potential(ρ_trial, grid, a_soft_ee)
        V_tot_trial = V_ext + V_H_trial
        Tpsi_trial = apply_kinetic(psi_rot, grid)
        Tmat_trial = dot_grid(psi_rot, Tpsi_trial, grid.dx)
        F_trial = free_energy(psi_rot, f_trial, Tmat_trial, ρ_trial, V_H_trial, grid, V_ext, kT, degeneracy)
        ...
        for _ in 1:10
            ...
            ρ_new = compute_density(psi_new_rot, f_new)
            V_H_new = hartree_potential(ρ_new, grid, a_soft_ee)
            V_tot_new = V_ext + V_H_new
            Tpsi_new = apply_kinetic(psi_new_rot, grid)
            Tmat_new = dot_grid(psi_new_rot, Tpsi_new, grid.dx)
            F_new = free_energy(psi_new_rot, f_new, Tmat_new, ρ_new, V_H_new, grid, V_ext, kT, degeneracy)

            # predicted change in μ
        ...
        Tpsi_new = apply_kinetic(psi_new_rot, grid)
        Tmat_new = dot_grid(psi_new_rot, Tpsi_new, grid.dx)
        Hpsi_new = apply_H(psi_new_rot, V_tot_new, grid)   # for next gradients
        ...
        if F_new > F_old
            @warn("Free energy is not decreasing")
        end
...
function do_main_calc(; psi=nothing, ebands=nothing)
