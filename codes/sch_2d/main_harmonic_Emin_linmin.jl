...
function main()
    ...
    for iter = 1:NiterMax
        ...
        @printf("linmin step %8d = %18.10f   %10.7e  nconv = %5d\n", iter, Ebands, diffE, nconv)

        if nconv >= Nstates_conv
            IS_CONVERGED = true
            @printf("Emin_linmin convergence: nconv = %5d in %5d iterations\n", nconv, iter)
            break
        end
    ...
    if !IS_CONVERGED
        @printf("\nWARNING: Emin_linmin is not converged after %d iterations\n", NiterMax)
    end
