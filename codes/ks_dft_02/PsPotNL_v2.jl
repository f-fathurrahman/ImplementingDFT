struct PsPotNL
    ...
    beta2atm::Array{Int64,1}
    beta2prj::Array{Int64,1}
...
function PsPotNL( atoms::Atoms, pspots::Array{PsPot_GTH,1}, grid )
    ...
    beta2atm = zeros(Int64,NbetaNL)
    beta2prj = zeros(Int64,NbetaNL)
    ibeta = 0
    for ia in 1:Natoms
        isp = atm2species[ia]
        psp = pspots[isp]
    ...
    if NbetaNL == 0
        # return dummy PsPotNL
        betaNL = zeros(Float64,1,1)
        return PsPotNL( 0, zeros(Int64,1,1,1,1),
            zeros(Int64,1), zeros(Int64,1), betaNL )
    end
    ...
    return PsPotNL( NbetaNL, prj2beta, beta2atm, beta2prj, betaNL )
