...
include("build_D2_matrix_LF1d_sinc.jl")
...
function main(N::Int64)
    x_min = -5.0
    x_max =  5.0
    x, h = init_LF1d_sinc_grid( x_min, x_max, N )
    fx = my_gaussian.(x)
    ...
    D2 = build_D2_matrix_LF1d_sinc(x, h, N)
    d2_fx = D2*fx
    ...
    plt.savefig("IMG_gaussian_"*string(N)*"_sinc.pdf")
